 ...
 * 
 * Usage: node scripts/test-all-transports.js
 */
    ...
    app.stdout.on('data', (data) => {
      ...
      if (output.includes('MCP server connected and ready')) {
        ...
        setTimeout(() => {
          ...
          try {
            const result = execSync('curl -s -X POST http://localhost:3235/mcp -H "Content-Type: application/json" -d @test-tools-list.json', { encoding: 'utf8' });
          ...
          try {
            const result = execSync('curl -s -X POST http://localhost:3235/mcp -H "Content-Type: application/json" -d @test-health-tool.json', { encoding: 'utf8' });
            const response = JSON.parse(result);
    ...
    app.stdout.on('data', (data) => {
      ...
      if (output.includes('MCP server connected and ready')) {
        ...
        setTimeout(() => {
          console.log('  📋 Testing tools/list...');
          try {
            const result = execSync('curl -s -X POST http://localhost:3236/mcp -H "Content-Type: application/json" -d @test-sse-tools-list.json', { encoding: 'utf8' });
            const response = JSON.parse(result);
          ...
          console.log('  🛠️  Testing tools/call...');
          try {
            const result = execSync('curl -s -X POST http://localhost:3236/mcp -H "Content-Type: application/json" -d @test-sse-health-tool.json', { encoding: 'utf8' });
            const response = JSON.parse(result);
...
async function runTests() {
  try {
    ...
    console.log('\n📊 Summary:');
    console.log('  🔗 HTTP Transport: Full MCP support with tools/list and tools/call');
    console.log('  📡 SSE Transport: Full MCP support with tools/list, tools/call, and info endpoint');
    console.log('  💬 STDIO Transport: Full MCP support via SDK handlers');
    
    console.log('\n🚀 Your MCP server is ready for AI agents!');
